...
static const unsigned char ZT_DEFAULT_WORLD[1] = {0};
// {
...
Topology::Topology(const RuntimeEnvironment* renv, void* tPtr) : RR(renv), _numConfiguredPhysicalPaths(0), _amUpstream(false)
{
    ...
    World emptyPlanet; // TYPE_NULL
    _planet = emptyPlanet; // Устанавливаем пустой planet напрямую
}
...
{
	// Блокируем добавление planet серверов
    if (newWorld.type() == World::TYPE_PLANET) {
        return false; // Не добавляем planet серверы
    }
